...
public class PerformanceTask {
	...
	double ypos=Math.sin(Math.toRadians(launchAngle)) * time * launchVelocity - 0.5 * 9.8 * Math.pow(xPos, yPos);
	boolean launch = true;
	...
	while (launch) {
	     ...
	     yPos=Math.sin(Math.toRadians(launchAngle)) * time * launchVelocity - 0.5 * 9.8 * Math.pow(ypos, xPos);
